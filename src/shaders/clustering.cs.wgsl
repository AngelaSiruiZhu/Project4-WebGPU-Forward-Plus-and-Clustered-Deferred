...
fn transformViewToNDC(viewSpacePoint: vec3f) -> vec2f {
    let clipSpacePos = camera.projMat * vec4f(viewSpacePoint, 1.0);
...
fn mapDepthToClusterLayer(viewSpaceDepth: f32) -> u32 {
    ...
    let depthSlices = f32(clusterSet.numClustersZ);
    let LogDepth = clamp((log(viewSpaceDepth / nearZ) / log(farZ / nearZ)), 0.0, 0.99999);
    return u32(floor(LogDepth * depthSlices));
}
...
fn assign(@builtin(global_invocation_id) gid: vec3u) {
    ...
    // 1D -> 3D
    let clustersPerLayer = gridSizeX * gridSizeY;
    let clusterZ = globalClusterIndex / clustersPerLayer;
    let layerOffset = globalClusterIndex - clusterZ * clustersPerLayer;
    let clusterY = layerOffset / gridSizeX;
    let clusterX = layerOffset - clusterY * gridSizeX;
    ...
    // pixel space bounds for the tile
    ...
    let clusterMaxDepth = nearZ * pow(farZ / nearZ, sliceUpperBound);
    ...
    for (var lightIndex = 0u; lightIndex < lightSet.numLights; lightIndex++) {
        ...
        let lightMinDepth = max(nearZ, lightDepth - lightSphereRadius);
        let lightMaxDepth = min(farZ, lightDepth + lightSphereRadius);
        if (lightMinDepth >= lightMaxDepth) { continue; }
        if (clusterMaxDepth <= lightMinDepth || clusterMinDepth >= lightMaxDepth) { continue; }


        let sphereLeftNDC = transformViewToNDC(vec3f(lightViewSpace.x - lightSphereRadius, lightViewSpace.y, lightViewSpace.z)).x;
        let sphereRightNDC = transformViewToNDC(vec3f(lightViewSpace.x + lightSphereRadius, lightViewSpace.y, lightViewSpace.z)).x;
        let sphereBottomNDC = transformViewToNDC(vec3f(lightViewSpace.x, lightViewSpace.y - lightSphereRadius, lightViewSpace.z)).y;
        let sphereTopNDC = transformViewToNDC(vec3f(lightViewSpace.x, lightViewSpace.y + lightSphereRadius, lightViewSpace.z)).y;
